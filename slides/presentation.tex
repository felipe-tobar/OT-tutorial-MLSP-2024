...
\bblue{Recall: GANs}
    ...
    \begin{figure}
        \centering
            \includegraphics[width=0.9\textwidth]{../img/gan_diag.pdf}
        \end{figure}
...
\begin{frame}{The Wasserstein distance}
    ...
    \begin{itemize}
        \item Does not need overlapping support
        \item Tells if the distributions are different, and also how much they are different!
    \end{itemize}
    \begin{figure}
        \centering
...
\begin{frame}{The Wasserstein space}
    ...
    \begin{itemize}
        \item In the space $\mathbb{W}_p(\mathbb{R}^d)$, we have $W_p(\mu_n, \mu) \to 0$ iff $\mu_n \to \mu$
        \item In the space $\mathbb{W}_p(\mathbb{R}^d)$, if we have $\mu_n \to \mu$ then $W_p(\mu_n, \mu) \to 0$ 
    \end{itemize}
    \bblue{Example} of a gradient flow on the support of a distribution that minimize $W_2^2(\mu_t, \mu)$:
    \begin{figure}
        \centering
...
\forall t \in [0,1],\quad \mu^{1\to 2}(t) = {(t T + (1-t)\text{Id})}_{\#} \mu_1
        ...
        \item \emph{there is an animation on the notebook + also for the L2 interpolation!}
    \end{itemize}
\end{minipage}
\hfill
\begin{minipage}{0.4\textwidth}
